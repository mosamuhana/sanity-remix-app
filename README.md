...
```sh
npx create-remix@latest --template mosamuhana/sanity-remix-app
```
...
The results of the Sanity query in `./app/routes/$slug.tsx` are parsed with a ArkType validator. This ensures that content is always returned in the expected shape at **run time**, and auto-generates Types which are fed from the `loader` into `useLoaderData()`.
...
### Import Data
```sh
 sanity dataset import ./resources/exported-data.tar.gz [your-project-dataset] --replace
```
