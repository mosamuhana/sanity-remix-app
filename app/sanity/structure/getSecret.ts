...
import { secretQuery } from "~/sanity/queries/utils";

export const SECRET_ID = `sanity.preview.secret`;

const tag = "preview.secret";

const randomSecret = () => Math.random().toString(36).slice(2);
...
export async function getSecret(
	...
	const secret = await client.fetch<string | null>(
		...
		secretQuery(createIfNotExists ? 60 * 30 : 60 * 60),
		{ id },
	);

	if (!secret && createIfNotExists) {
    try {
      const newSecret = createIfNotExists === true ? randomSecret() : createIfNotExists();
